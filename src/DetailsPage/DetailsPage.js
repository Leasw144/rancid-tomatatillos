...
class DetailsPage extends Component {
  ...
  getMovieDetails = async (movieId) => {
    try {
      ...
      this.setState({ error: 'An error occurred getting movie details.' })
    }
  }
  ...
  render() {
    ...
    return (
      <section className='DetailsPage'>
        <article className='backdrop-parent'>
          {
            this.props.error && 
          ...
          />
          {
            this.state.movieInfo.tagline && 
            <p className='tag-line'>{this.state.movieInfo.tagline}</p>
          }
          {
            this.props.userName && 
            <form className='comment-form'>
              ...
              <button 
                type="button" 
                onClick={() => this.postUserComment(this.state.userComment)}>
                  Submit Comment
              </button>
            </form>
          }
